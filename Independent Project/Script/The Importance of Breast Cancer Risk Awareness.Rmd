...
ggplot(data = subset(barcancer, !is.na(age)),# drop all na's that were going to be included into the plot
  ...
  theme(plot.title = element_text(size = 20, hjust = 0.5, color ="darkgoldenrod1"), # title size and color
        plot.title.position = "plot", # title adjustments
...
```{r}
  ...
  labs(title = "Correlation of the two Frequencies, Average of Radius and Texture within Breast Cancer Patients", subtitle = "There are two groups of breast cancer patients some with benign tumors and the other with malignant tumors.", # title and subtitles x and y axes 
       x="Average Texture",
       y="Average Radius") +
  theme_classic() +
   transition_states( 
        texture_mean, # what are we animating by
        transition_length = 2, #The relative length of the transition.
        state_length = 1) + # The length of the pause between transitions
        ease_aes("bounce-in-out") + # type of animation
  theme(plot.title = element_text(size = 20, hjust = 0.5, color =""), # title size and color
        plot.title.position = "plot", # title adjustments
        plot.subtitle = element_text(size =10,hjust = 0.5, lineheight = 1.1, color = "darkseagreen4"), # subtitle size and color
        axis.title = element_text(size=20,color = "darkseagreen", hjust = 0.5), # title adjustments
        axis.text = element_text(size = 15,color = "darkorchid4",hjust = 0.5), # text adjustments
        panel.grid = element_line(linewidth = 0.35, linetype = "dotted", color = "darkorchid4"), # background gridlines
        plot.margin = unit(c(1.0, 1.0, 1.0, 1.0), "cm"), # margin adjustments
        plot.background = element_rect(color = "white", fill ="white")) # background color
        
        anim_save(here("Independent Project","Output","hexplotgif.gif"))
...
ggplot(data = cancer, aes(x= class,
  ...
  geom_density_ridges(alpha = 0.5,
                      ...
                      size = 0.2) + 
  theme_minimal() +
  labs(x = "Average Texture", y = "Average Radius", title = "Correlation of the two Frequencies, Average of Radius and Texture within Breast Cancer Patients", subtitle = "There are two groups of breast cancer patients some with benign tumors and the other with malignant tumors.") + # title and subtitles + x and y axes 
  transition_states( 
     age, # what are we animating by
  ...
  anim_save(here("Independent Project","testingif.gif"))
